...
def build(REV_GIT_PATH, REV_REPORTS_PATH, filePref):
	...
	if((not built) and hasGradlew):
		...
		built = runBuild("chmod +x " + gradlew + " && "+gradlew + " build -p " +REV_GIT_PATH, REV_REPORTS_PATH + "/" + filePref + "build_gradlew.txt")
		lastBuildRun = "Gradle"
	...
	if((not built) and hasGradle):
		...
		built = runBuild("gradle build -p " +REV_GIT_PATH, REV_REPORTS_PATH + "/" + filePref + "build_gradle.txt")
		lastBuildRun = "Gradle"
	...
	if((not built) and hasAnt):
		...
		built = runBuild("ant build -buildfile "+ REV_GIT_PATH + "/build.xml", REV_REPORTS_PATH + "/" + filePref + "build_ant.txt")
		lastBuildRun = "Ant"
...
def runJoanaForSpecificRevs():
	...
	revList = readLines(CA_PATH + "/revList08-02_2")
